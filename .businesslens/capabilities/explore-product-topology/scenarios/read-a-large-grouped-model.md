...
routes:
  local: Local
steps:
  - text: The Developer opens the Graph of a collection holding many resources
    kind: actor
    actor: developer
    entities:
  ...
  - text: The Product shows each branch with its count and makes larger branches explicitly expandable
    ...
    contexts:
      local:
        place: local-report-web::resource-collection
  - text: The Developer expands a branch and opens one resource from it
    kind: actor
    actor: developer
    entities: []
    contexts:
...
A Product Model holds more resources than one Graph can draw at a readable
size, and the Developer wants one of them.

## Outcome

The Developer can reach any resource the Graph branches without losing track of
the visible scope, and reads its connections on its own page.
