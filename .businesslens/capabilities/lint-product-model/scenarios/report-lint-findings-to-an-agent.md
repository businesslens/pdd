...
steps:
  ...
  - text: The AI agent asks for the check in machine-readable form, from outside the repository it is inspecting
    ...
    contexts:
      terminal:
        place: businesslens-cli
  - text: The Product returns whether the model passed, every error, every warning, and a count of each entity kind
    kind: product
    contexts:
      terminal:
        place: businesslens-cli
...
The agent holds the structural findings and the entity counts as data. It has
