...
4. If `.businesslens/` already contains a complete model, do not replace it
...
6. Form repository-backed hypotheses for actors, experiences, domains,
   features, business rules, and journeys. Ask the user only about material
   ambiguity that repository evidence cannot resolve.
7. Trace behavior from entry points through handlers/services, persistence,
   ...
   - features expressing stable capabilities;
   - business rules stating reusable constraints and policies;
   - journeys expressing stable user or operator goals;
   - scenarios covering primary, permission, validation, conflict, and
     external-failure paths when evidenced, with decision points where
