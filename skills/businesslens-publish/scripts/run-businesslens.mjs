...
if (!requestedRoot) fail('Usage: run-businesslens.mjs --root <repository> validate|publish [options]')

const commandArgs = args.filter((_, index) => index !== rootIndex && index !== rootIndex + 1)
if (!['validate', 'publish'].includes(commandArgs[0])) {
  fail('The isolated BusinessLens runner supports only validate and publish.')
}
...
if (commandArgs[0] !== 'publish') delete env.BUSINESSLENS_API_KEY
