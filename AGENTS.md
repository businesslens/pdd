...
- This repository authors the documentation with groups Get started, Product
  Model (one page per top-level resource type, plus the generated Vocabulary
  index), Integrations (one page per thing you integrate with), Skills (one
...
- **A term is defined in the frontmatter of the page that explains it**, as a
  `terms:` entry with a one-line `definition` and the `anchor` it sits at. `on`
  scopes a word two pages genuinely own — Trigger belongs to both Scenario
  types — and only ever disambiguates the slug. Everything reading them is
  generated by `npm run vocabulary`: `docs/vocabulary.md`, which is an index
  that defines nothing of its own, and the report viewer's registry. Both are
  committed and `npm run check` fails while either disagrees with `docs/`. This
...
- **A definition that leans on another term links to it**, resolved at
  ...
  ordinary English word — skips a match at the start of a definition, where a
  capital says nothing, and links nothing for a word two terms share. Write a
  definition so it reads without the links, then let them carry the reader who
  needs one. `aliases` lists the shorthand the documentation writes for a term —
  "Rule" for a Business Rule — so those link too.
