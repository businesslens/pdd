...
It drafts the model and coverage assessment, asks for approval, writes only
inside `.businesslens/`, and runs structural lint in an isolated runner.

Map never executes target code and never silently replaces a mature model.
Optional codeRefs are useful tracked-file bookmarks, not proof.
