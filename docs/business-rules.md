...
description: Durable assertions that apply to behavior — Capabilities, Journeys and their Scenarios — or to Contexts naming Interfaces, Experiences, or Screens.
section: open-source
group: Product Model
order: 17
---
...
Contexts. Other resources do not copy Rule IDs, so one
...
a Rule's targets resolve to a single behavioral resource with no `contexts`
narrowing them.

Write something that must remain true, not a sequential step.

A Rule must have at least one target. Target a behavioral resource when the Rule
governs that behavior. Add `contexts` to narrow a resource target, or use a
...
| --- | --- | --- |
| `appliesTo` | yes | Give at least one unique behavioral or direct context target. A behavioral target's `type` is `capability`, `capability-scenario`, `journey`, or `journey-scenario`; a direct target's is `context`. No other resource type is targetable. |
...
A resource target without `contexts` applies everywhere that resource is
