 ...
 * module is the one place that reads them generically, so a design can offer
 * the same filter and group-by controls on every kind without a per-kind
 * branch for each relation.
 *
 * Nothing here derives new relations: a facet is only ever an id array the
 * projection already holds, which keeps "group by Capability" a restatement of
 * the model rather than an interpretation of it.
 */
...
export function facetKindsFor(kind: ReportEntityKind): ReportEntityKind[] {
  switch (kind) {
    ...
    case 'capability': return ['domain', 'interface', 'experience', 'journey', 'screen', 'rule']
    case 'journey': return ['actor', 'interface', 'experience', 'capability', 'domain', 'screen', 'rule']
    case 'scenario': return ['journey', 'screen', 'rule']
