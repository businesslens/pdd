...
/**
 * Orbit — the sitemap is the centre of the report.
 *
 * The industry calls this artifact a sitemap and usually draws it as a
 * top-down tree. HOME ("Sitemap") is that tree — the Product at the root,
 * Interfaces, Experiences and Screens one level per click depth — with a
 * toggle to the centralized radial drawing of the same hierarchy, where
 * distance from the core is the first thing a reader sees. The left rail
 * lists every availability scope
 * from `workspace.pairs` ("Whole product", each Interface directly, each
 * Interface › Experience); picking one lights its region of the map and
 * opens a scope panel of exactly what exists there. A Journey overlay lights
 * the Screens one promise passes through. Compare mode swaps the map for a
 * factual set-membership delta of two contexts. Tabs complete the report:
 * Promises (journey cards ⇄ table, full scenario reading), Abilities
 * (domain-grouped capabilities + two named matrices), Constraints (rule
 * impact, direct vs derived), About (identity, coverage, references). Any
 * entity opens the shared inspector slideover: full BlrEntityDetail with a
 * "Map" tab to the contextual BlrTopology.
 */
import { h } from 'vue'
...
type TabId = 'sitemap' | 'promises' | 'abilities' | 'constraints' | 'about'

const TABS: TabsItem[] = [
  { value: 'sitemap', label: 'Sitemap', icon: 'i-lucide-orbit' },
...
/* Journey overlay: light the Screens one promise passes through. */
const overlayJourneyId = ref('')
...
/* Two drawings of the same tree: the industry top-down default, or centralized. */
const sitemapLayout = ref<'tree' | 'radial'>('tree')
const LAYOUT_TABS: TabsItem[] = [
...
const sitemap = computed(() => {
  ...
  const journey = overlayJourney.value
  let emphasizeScreenIds: ReadonlySet<string> | null = null
  if (scope && journey) {
    const scoped = new Set(scope.screenIds)
    emphasizeScreenIds = new Set(journey.screenIds.filter(id => scoped.has(id)))
  } else if (scope) {
    emphasizeScreenIds = new Set(scope.screenIds)
  } else if (journey) {
    emphasizeScreenIds = new Set(journey.screenIds)
  }
...
<template>
  <div class="flex h-full min-h-0 flex-col">
    ...
    <header class="flex shrink-0 flex-wrap items-center gap-x-3 gap-y-1 border-b border-default px-4 py-2">
      ...
      <UIcon v-else name="i-lucide-orbit" class="size-5 shrink-0 text-primary" />
      <div class="min-w-0">
    ...
    <div class="relative flex min-h-0 flex-1">
      <!-- ============ SITEMAP: the centre of the report ============ -->
      <div v-if="tab === 'sitemap'" class="flex min-h-0 min-w-0 flex-1">
        ...
        <nav class="blr-pane w-72 shrink-0 border-e border-default">
          <div class="space-y-1 p-3">
            ...
            <button
              type="button"
              class="ob-context ob-context--direct"
            ...
            <div v-for="group in railGroups" :key="group.iface.id" class="pt-2">
              ...
              <button
                ...
                :key="entry.pair.key"
                type="button"
                class="ob-context ob-context--nested"
        ...
        <div class="flex min-h-0 min-w-0 flex-1 flex-col">
          ...
          <div class="relative min-h-0 flex-1">
            ...
            <template v-else>
              ...
              />
              <div class="absolute top-2 left-2 z-10 flex items-center gap-1.5 rounded-full border border-default bg-elevated/90 py-1 ps-3 pe-1 shadow-sm">
                <span class="blr-field shrink-0">Journey overlay</span>
                <USelect
                  v-model="overlayJourneyId"
                  :items="overlayItems"
                  placeholder="Light one Journey"
                  size="xs"
                  class="w-48"
                ...
                <UButton
                  ...
                  color="neutral"
                  variant="ghost"
                  :title="`Open ${overlayJourney.title}`"
                  @click="inspect(overlayJourney.id)"
                />
                <UButton
                  v-if="overlayJourney"
                  icon="i-lucide-x"
                  ...
                  @click="overlayJourneyId = ''"
                />
              </div>
              <div class="absolute top-2 right-2 z-10 rounded-full border border-default bg-elevated/90 p-0.5 shadow-sm">
                <UTabs v-model="sitemapLayout" :items="LAYOUT_TABS" :content="false" color="neutral" size="xs" />
              </div>
              <div class="absolute bottom-2 left-1/2 z-10 -translate-x-1/2 rounded-full border border-default bg-elevated/90 px-3 py-1 text-sm text-muted shadow-sm">
                {{ sitemapLayout === 'tree'
                  ? 'Levels step down from the Product: Interfaces, then Experiences, then their Screens.'
                  : 'Rings step outward from the Product core: Interfaces, then Experiences, then their Screens.' }}
              </div>
              <div
                v-if="compareOn"
      ...
      <div v-else-if="tab === 'promises'" class="flex min-h-0 min-w-0 flex-1 flex-col">
        ...
        >
          ...
          <UButton size="xs" color="neutral" variant="ghost" label="View on Sitemap" @click="tab = 'sitemap'" />
        </div>
        ...
        <div v-if="openJourney" class="blr-pane flex-1">
          <div class="mx-auto max-w-4xl space-y-6 p-4 sm:p-6">
            ...
            <section class="space-y-1.5 rounded-xl border border-default bg-default p-4">
              <p class="blr-field">Available in — click to scope the Sitemap</p>
              <div class="flex flex-wrap gap-1.5">
                <UButton
          ...
          <div class="mx-auto max-w-6xl space-y-8 p-4 sm:p-6">
            ...
            <section class="space-y-2">
              ...
              <div class="overflow-x-auto rounded-xl border border-default bg-default p-3.5">
                <table class="ob-matrix">
                  <thead>
                    <tr>
                      <th class="text-start"><span class="blr-field">Capability</span></th>
                      <th v-for="pair in workspace.pairs" :key="pair.key">
                        <button type="button" class="ob-colhead" :title="`Scope the Sitemap to ${pairLabel(pair)}`" @click="goToContext(pair.key)">
                          {{ pairLabel(pair) }}
                        </button>
                      </th>
                    </tr>
                  ...
                  <thead>
                    <tr>
                      ...
                      <th v-for="journey in workspace.journeys" :key="journey.id">
                        <button type="button" class="ob-colhead" :title="`Open ${journey.title}`" @click="inspect(journey.id)">
                          {{ journey.title }}
                        </button>
                      </th>
                    </tr>
            ...
            <div v-if="matrixDetail" class="flex items-start gap-3 rounded-xl border border-default bg-default px-4 py-3">
              ...
              <div class="min-w-0 flex-1 space-y-2">
                <p class="text-sm leading-6 text-default">
                  <button type="button" class="ob-inline" @click="inspect(matrixDetail.capability.id)">{{ matrixDetail.capability.title }}</button>
                  <template v-if="matrixDetail.matrix === 'reach' && matrixDetail.pair">
                    ...
                    <button type="button" class="ob-inline" @click="goToContext(matrixDetail.pair.key)">{{ pairLabel(matrixDetail.pair) }}</button>.
                  </template>
                  <template v-else-if="matrixDetail.journey">
                    {{ matrixDetail.on ? ' is used by ' : ' is not used by ' }}
                    <button type="button" class="ob-inline" @click="inspect(matrixDetail.journey.id)">{{ matrixDetail.journey.title }}</button>.
                  </template>
                </p>
                <BlrLinks
            ...
            <section class="space-y-1.5">
              ...
              <div v-if="selectedRule.availability.length" class="flex flex-wrap gap-1.5">
                <UButton
                  ...
                  class="rounded-full"
                  :label="pairLabel(pair)"
                  :title="`Scope the Sitemap to ${pairLabel(pair)}`"
                  @click="goToContext(pair.key)"
                />
              </div>
...
/* Rail entries: solid structure line for Interface-level scopes, dashed for
   Experience floors — echoing the sitemap's ring order. */
.ob-context {
  display: block;
  width: 100%;
...
.ob-matrix tbody th button:hover {
  color: var(--ui-primary);
}
