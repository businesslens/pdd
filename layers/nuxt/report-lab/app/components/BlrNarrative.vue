  ...
  <div class="nar pb-24">
    ...
    <nav class="blr-no-print sticky top-[var(--blr-chrome)] z-30 -mb-px border-b border-default bg-default/90 backdrop-blur xl:hidden" aria-label="Chapters">
      <div class="flex gap-1 overflow-x-auto py-2">
        <button v-for="(chapter, index) in CHAPTERS" :key="chapter.id" type="button" class="flex shrink-0 items-baseline gap-1.5 rounded-full border px-2.5 py-1 text-xs transition" :class="activeChapter === chapter.id ? 'border-accented bg-elevated text-highlighted' : 'border-transparent text-dimmed hover:text-toned'" :aria-current="activeChapter === chapter.id ? 'true' : undefined" @click="scrollToChapter(chapter.id)">
          <span class="font-mono text-[10px] tabular-nums">{{ index + 1 }}</span>{{ chapter.title }}
        </button>
      </div>
    </nav>
    ...
    <div class="xl:grid xl:grid-cols-[minmax(0,1fr)_11rem] xl:gap-12">
      <article class="mx-auto w-full min-w-0 max-w-4xl space-y-20 pt-8">
        ...
        <section id="nar-chapter-product" data-chapter="product" class="nar-chapter space-y-8">
          <header class="nar-head">
            <span class="nar-num">1</span><div class="min-w-0"><h2 class="nar-title">{{ CHAPTERS[0].title }}</h2><p class="nar-question">{{ CHAPTERS[0].question }}</p></div>
          </header>
            ...
            <div class="flex items-start gap-4">
              ...
              <div class="min-w-0 space-y-2">
                <div class="flex flex-wrap items-center gap-2">
                  <code class="rounded bg-muted px-1.5 py-0.5 font-mono text-[11px] text-dimmed">{{ workspace.identity.id }}</code>
                  <UBadge v-if="workspace.identity.categoryLabel" color="neutral" variant="subtle" size="sm">{{ workspace.identity.categoryLabel }}</UBadge>
                </div>
                <h1 class="text-4xl font-light tracking-tight text-highlighted">{{ workspace.identity.title }}</h1>
                <p class="text-lg leading-relaxed text-toned">{{ workspace.identity.summary }}</p>
              </div>
            </div>
            <BlrProse :text="workspace.identity.description" size="base" />
            <div v-if="workspace.identity.intent" class="space-y-1.5">
              <p class="nar-label">Intent</p>
          ...
          <div class="space-y-2">
            <div class="flex flex-wrap items-baseline gap-x-7 gap-y-3 border-y py-4 blr-hairline">
              <div v-for="figure in figures" :key="figure.meta.kind" class="flex items-baseline gap-2">
                <span class="text-2xl font-light tabular-nums text-highlighted">{{ figure.value }}</span>
                <span class="inline-flex items-center gap-1 text-xs text-dimmed"><BlrKind :kind="figure.meta.kind" :labelled="false" size="xs" />{{ figure.meta.plural }}</span>
              </div>
            </div>
            <p class="text-xs leading-relaxed text-dimmed">Depth, derived from the authored model: {{ depthSentence }}</p>
          </div>

          <div class="space-y-3 rounded-xl border border-default p-5">
            <div class="flex flex-wrap items-center gap-2">
              <p class="nar-label">Coverage</p>
            ...
            <div v-if="workspace.coverage.method.length" class="flex flex-wrap items-baseline gap-1.5 text-xs">
              <span class="nar-label">Method</span>
              <span v-for="item in workspace.coverage.method" :key="item" class="rounded-full border border-default px-2 py-0.5 text-toned">{{ item }}</span>
            </div>
            <div v-if="workspace.coverage.sourceAreas.length" class="flex flex-wrap items-baseline gap-1.5 text-xs">
              <span class="nar-label">Source areas</span>
            ...
            <div v-if="workspace.coverage.unmapped.length" class="space-y-1 text-xs">
              <p class="nar-label">Not yet mapped</p>
              <ul class="list-disc space-y-0.5 ps-5 text-dimmed marker:text-dimmed"><li v-for="(item, index) in workspace.coverage.unmapped" :key="index">{{ item }}</li></ul>
            </div>
            <div v-if="workspace.coverage.limitations.length || workspace.identity.limitations.length" class="space-y-1 text-xs">
              <p class="nar-label">Known limitations</p>
              <ul class="list-disc space-y-0.5 ps-5 text-dimmed marker:text-dimmed">
          ...
          <div class="space-y-3">
            <div v-if="workspace.identity.tags.length" class="flex flex-wrap items-baseline gap-1.5">
              <span class="nar-label">Tags</span>
              <UBadge v-for="tag in workspace.identity.tags" :key="tag" color="neutral" variant="outline" size="sm">{{ tag }}</UBadge>
            </div>
            <p v-if="workspace.identity.authors.length" class="text-sm text-toned">
              <span class="nar-label me-2">Authors</span>
            ...
            <div v-if="workspace.identity.supportingContent" class="space-y-1.5">
              <p class="nar-label">Supporting context</p>
        ...
        <section id="nar-chapter-access" data-chapter="access" class="nar-chapter space-y-8">
          <header class="nar-head">
            <span class="nar-num">2</span><div class="min-w-0"><h2 class="nar-title">{{ CHAPTERS[1].title }}</h2><p class="nar-question">{{ CHAPTERS[1].question }}</p></div>
          </header>

          <div class="space-y-5">
            <h3 class="nar-sub">2.1 The people and systems involved</h3>
            <div v-for="actor in workspace.actors" :id="`nar-${actor.id}`" :key="actor.id" class="nar-anchor nar-block space-y-2.5" :class="flashId === actor.id && 'nar-flash'">
              <div class="flex flex-wrap items-center gap-2">
                ...
                <h4 class="text-lg text-highlighted">{{ actor.title }}</h4>
                <UBadge color="neutral" variant="subtle" size="sm">{{ actor.actorKind }} · {{ actor.relationship }}</UBadge>
              </div>
              <BlrProse :text="actor.lead" />
              <div v-if="actor.intent" class="space-y-1"><p class="nar-label">Intent</p><BlrProse :text="actor.intent" /></div>
              <div class="space-y-1">
              ...
              <BlrProse v-if="actor.supportingContent" :text="actor.supportingContent" class="text-dimmed" />
              <BlrRefs :references="actor.references" />
            </div>
            <p v-if="!workspace.actors.length" class="text-sm italic text-dimmed">The model names no Actors.</p>
          </div>

          <div class="space-y-5">
            <h3 class="nar-sub">2.2 The doors: Interfaces<template v-if="workspace.experiences.length"> and their Experiences</template></h3>
            <div v-for="item in workspace.interfaces" :id="`nar-${item.id}`" :key="item.id" class="nar-anchor nar-block space-y-2.5" :class="flashId === item.id && 'nar-flash'">
              <div class="flex flex-wrap items-center gap-2">
                <BlrKind kind="interface" :labelled="false" /><h4 class="text-lg text-highlighted">{{ item.title }}</h4>
              </div>
              <BlrProse :text="item.lead" />
              <BlrAvail :pairs="[]" :entry-points="item.entryPoints" label="Entry points" />
              <div class="space-y-1"><p class="nar-label">Capability boundary</p><BlrProse :text="item.capabilityBoundary" /></div>
              <div class="space-y-1">
              ...
              <p class="text-xs text-dimmed">Derived from declared availability: {{ item.screenIds.length }} Screens, {{ item.capabilityIds.length }} Capabilities and {{ item.journeyIds.length }} Journeys reachable through this Interface.</p>
            ...
            <div v-for="item in workspace.experiences" :id="`nar-${item.id}`" :key="item.id" class="nar-anchor nar-block ms-4 space-y-2.5 border-s-2 border-muted ps-4" :class="flashId === item.id && 'nar-flash'">
              <div class="flex flex-wrap items-center gap-2">
                ...
                <h4 class="text-lg text-highlighted">{{ item.title }}</h4>
              ...
              <p class="text-xs text-dimmed">Derived from declared availability: {{ item.screenIds.length }} Screens, {{ item.capabilityIds.length }} Capabilities and {{ item.journeyIds.length }} Journeys scoped to this Experience.</p>
              <BlrRefs :references="item.references" />
            </div>
            <p v-if="!workspace.experiences.length" class="text-sm italic text-dimmed">No Interface declares Experiences — every access context is the Interface itself.</p>
          </div>

          <figure class="space-y-2">
            <figcaption class="nar-caption">How the access contexts compare — every count is derived from declared availability.</figcaption>
              ...
              <table class="nar-table">
                ...
                <tbody>
                  <tr v-for="row in accessRows" :key="row.key">
                    ...
                    <td class="text-dimmed">{{ row.type }}</td><td class="text-dimmed">{{ row.access }}</td><td class="text-toned">{{ row.actors || '—' }}</td>
        ...
        <section id="nar-chapter-surface" data-chapter="surface" class="nar-chapter space-y-8">
          <header class="nar-head">
            <span class="nar-num">3</span><div class="min-w-0"><h2 class="nar-title">{{ CHAPTERS[2].title }}</h2><p class="nar-question">{{ CHAPTERS[2].question }}</p></div>
          </header>
          ...
          <p v-else class="text-sm leading-relaxed text-dimmed">The model declares no Screens: none of its Interfaces is a graphical surface, so this Product has no visible surface to map. Its behaviour is told entirely through the promises in the next chapter.</p>

          <div v-for="(section, index) in surfaceSections" :key="section.productInterface.id" class="space-y-4">
            <h3 class="nar-sub">3.{{ index + 1 }} Screens of {{ section.productInterface.title }}</h3>
            <template v-if="section.screens.length">
              <div v-for="entry in section.screens" :id="entry.anchor ? `nar-${entry.screen.id}` : undefined" :key="entry.screen.id" class="nar-anchor nar-block space-y-2.5" :class="flashId === entry.screen.id && entry.anchor && 'nar-flash'">
                <div class="flex flex-wrap items-center gap-2">
                  ...
                  <h4 class="text-base font-medium text-highlighted">{{ entry.screen.title }}</h4>
                ...
                <p class="text-xs text-dimmed">Presents {{ entry.screen.information.length }} information items · {{ entry.screen.actions.length }} actions · {{ entry.screen.states.length }} states<template v-if="entry.screen.capabilityIds.length"> · exposes {{ titlesOf(entry.screen.capabilityIds) }}</template></p>
            ...
            <p v-else class="text-sm italic leading-relaxed text-dimmed">“{{ section.productInterface.title }}” declares no Screens — it is not a graphical surface, and that is a fact about its interaction contract, not a gap in the model.</p>
          </div>
        </section>
        ...
        <section id="nar-chapter-promises" data-chapter="promises" class="nar-chapter space-y-10">
          <header class="nar-head">
            <span class="nar-num">4</span><div class="min-w-0"><h2 class="nar-title">{{ CHAPTERS[3].title }}</h2><p class="nar-question">{{ CHAPTERS[3].question }}</p></div>
          </header>

          <div v-if="workspace.scenarioKinds.length" class="flex flex-wrap items-baseline gap-1.5">
            <span class="nar-label">Scenario kinds</span>
            <UTooltip v-for="kind in workspace.scenarioKinds" :key="kind.id" :text="kind.description || kind.name">
              <span class="inline-flex items-center gap-1.5 rounded-full border border-default px-2 py-0.5 text-xs text-toned">
                {{ kind.name }}<span class="font-mono text-[10px] tabular-nums text-dimmed">{{ kind.count }}</span>
              </span>
            </UTooltip>
          </div>
          ...
          <section v-for="(entry, journeyIndex) in journeyChapters" :id="`nar-${entry.journey.id}`" :key="entry.journey.id" class="nar-anchor space-y-6" :class="flashId === entry.journey.id && 'nar-flash'">
            ...
            <header class="space-y-3 border-t pt-6 blr-hairline">
              <p class="nar-subnum">4.{{ journeyIndex + 1 }} · Journey</p>
              <h3 class="text-2xl tracking-tight text-highlighted">{{ entry.journey.title }}</h3>
              <BlrProse :text="entry.journey.lead" size="base" />
              <div v-if="entry.journey.intent" class="space-y-1"><p class="nar-label">Intent</p><BlrProse :text="entry.journey.intent" /></div>
              ...
              <p class="text-xs text-dimmed">{{ entry.scenarios.length }} scenarios · {{ entry.journey.stepCount }} authored steps<template v-if="entry.journey.domainIds.length"> · touches {{ titlesOf(entry.journey.domainIds) }} (derived)</template></p>
              <div class="space-y-2">
                <p class="nar-label">Capabilities, Screens and Rules woven through it — tap any to preview</p>
                <div class="flex flex-wrap gap-1.5">
                  <button v-for="chip in resolveIds(journeyChipIds(entry.journey))" :key="chip.id" type="button" class="nar-chip" :class="isOpen(chipKey('j', entry.journey.id, chip.id)) && 'nar-chip--open'" @click="toggle(chipKey('j', entry.journey.id, chip.id))">
                    <BlrKind :kind="chip.kind" :labelled="false" size="xs" />{{ chip.title }}
                    <UIcon :name="isOpen(chipKey('j', entry.journey.id, chip.id)) ? 'i-lucide-chevron-up' : 'i-lucide-chevron-down'" class="size-3 text-dimmed" />
                  </button>
                </div>
            ...
            <div v-for="(scenario, scenarioIndex) in entry.scenarios" :id="`nar-${scenario.id}`" :key="scenario.id" class="nar-anchor nar-scenario space-y-3" :class="flashId === scenario.id && 'nar-flash'">
              <div class="flex flex-wrap items-center gap-2">
                <p class="nar-subnum">4.{{ journeyIndex + 1 }}.{{ scenarioIndex + 1 }}</p>
                <h4 class="text-base font-medium text-highlighted">{{ scenario.title }}</h4>
                <UBadge color="neutral" variant="subtle" size="sm">{{ scenario.kindName }}</UBadge>
              </div>
              <div class="space-y-1"><p class="nar-label">When</p><BlrProse :text="scenario.trigger" /></div>
              <div class="space-y-1">
                <p class="nar-label">Steps · {{ scenario.steps.length }}</p>
                <ol class="list-decimal space-y-1.5 ps-6 text-sm leading-relaxed text-toned marker:font-mono marker:text-xs marker:text-dimmed">
              ...
              <div v-for="(point, pointIndex) in scenario.decisionPoints" :key="pointIndex" class="ms-6 space-y-2 border-s-2 border-muted ps-4">
                ...
                <BlrProse :text="point.question" class="text-dimmed" />
                <ul class="space-y-1.5">
                  <li v-for="(branch, branchIndex) in point.branches" :key="branchIndex" class="flex flex-wrap items-baseline gap-2 text-sm">
                    <span class="rounded bg-muted px-1.5 py-0.5 font-mono text-xs text-toned">{{ branch.condition }}</span>
                    <UIcon name="i-lucide-arrow-right" class="size-3.5 self-center text-dimmed" />
                    <span class="text-toned">{{ branch.outcome }}</span>
                  </li>
                </ul>
              </div>
              <div class="space-y-1"><p class="nar-label">Outcome</p><BlrProse :text="scenario.outcome" /></div>
              <aside v-if="scenario.edgeCases.length" class="nar-margin-note">
                <p class="nar-label">Edge cases · {{ scenario.edgeCases.length }}</p>
                <ul class="mt-1 space-y-1 text-xs leading-relaxed text-dimmed">
              ...
              <div v-if="scenarioChipIds(scenario).length" class="space-y-2">
                <div class="flex flex-wrap gap-1.5">
                  <button v-for="chip in resolveIds(scenarioChipIds(scenario))" :key="chip.id" type="button" class="nar-chip" :class="isOpen(chipKey('s', scenario.id, chip.id)) && 'nar-chip--open'" @click="toggle(chipKey('s', scenario.id, chip.id))">
                    <BlrKind :kind="chip.kind" :labelled="false" size="xs" />{{ chip.title }}
                    <UIcon :name="isOpen(chipKey('s', scenario.id, chip.id)) ? 'i-lucide-chevron-up' : 'i-lucide-chevron-down'" class="size-3 text-dimmed" />
                  </button>
                </div>
              ...
              <BlrProse v-if="scenario.supportingContent" :text="scenario.supportingContent" class="text-dimmed" />
              <BlrRefs :references="scenario.references" />
            </div>
            <p v-if="!entry.scenarios.length" class="text-sm italic text-dimmed">This Journey has no authored Scenarios yet — the promise is declared but not cased out.</p>
          ...
          <figure class="space-y-2">
            <figcaption class="nar-caption">The promises compared — counts are derived from the authored model, never a ranking.</figcaption>
            <div class="overflow-x-auto">
              <table class="nar-table">
                <thead>
                  <tr>
                ...
                <tbody>
                  <tr v-for="row in journeyRows" :key="row.journey.id">
                    ...
                    <td class="text-toned">{{ row.actors || '—' }}</td><td class="text-dimmed">{{ row.access || '—' }}</td>
        ...
        <section id="nar-chapter-abilities" data-chapter="abilities" class="nar-chapter space-y-8">
          <header class="nar-head">
            <span class="nar-num">5</span><div class="min-w-0"><h2 class="nar-title">{{ CHAPTERS[4].title }}</h2><p class="nar-question">{{ CHAPTERS[4].question }}</p></div>
          </header>
          ...
          <div v-for="(section, sectionIndex) in domainSections" :key="section.key" class="space-y-4">
            <div :id="section.domain ? `nar-${section.domain.id}` : undefined" class="nar-anchor space-y-2" :class="section.domain && flashId === section.domain.id && 'nar-flash'">
              <h3 class="nar-sub">5.{{ sectionIndex + 1 }} {{ section.title }}</h3>
              <template v-if="section.domain">
                <BlrProse :text="section.domain.lead" />
                <p class="text-xs text-dimmed">Derived reach: {{ section.domain.journeyIds.length }} Journeys, {{ section.domain.screenIds.length }} Screens and {{ section.domain.ruleIds.length }} Rules touch this Domain through its Capabilities.</p>
                <BlrRefs :references="section.domain.references" />
              </template>
              <p v-else class="text-sm italic text-dimmed">These Capabilities belong to no Domain — the model declares them free-standing.</p>
            </div>
            <div v-for="capability in section.capabilities" :id="`nar-${capability.id}`" :key="capability.id" class="nar-anchor nar-block space-y-2.5" :class="flashId === capability.id && 'nar-flash'">
              <div class="flex flex-wrap items-center gap-2">
                <BlrKind kind="capability" :labelled="false" /><h4 class="text-base font-medium text-highlighted">{{ capability.title }}</h4>
              </div>
              <BlrProse :text="capability.lead" />
              <div v-if="capability.intent" class="space-y-1"><p class="nar-label">Intent</p><BlrProse :text="capability.intent" /></div>
              <div class="space-y-1">
              ...
              <BlrProse v-if="capability.supportingContent" :text="capability.supportingContent" class="text-dimmed" />
              <BlrRefs :references="capability.references" />
            </div>
          </div>
          <p v-if="!workspace.capabilities.length" class="text-sm italic text-dimmed">The model declares no Capabilities.</p>

          <template v-if="workspace.capabilities.length">
            <figure v-if="workspace.journeys.length" class="space-y-2">
              <figcaption class="nar-caption">
                <span class="font-medium text-toned">Matrix — Which Product promises depend on each Capability?</span>
            ...
            <figure v-if="workspace.pairs.length" class="space-y-2">
              <figcaption class="nar-caption">
                <span class="font-medium text-toned">Matrix — Where can each Capability be reached?</span>
        ...
        <section id="nar-chapter-constraints" data-chapter="constraints" class="nar-chapter space-y-10">
          <header class="nar-head">
            <span class="nar-num">6</span><div class="min-w-0"><h2 class="nar-title">{{ CHAPTERS[5].title }}</h2><p class="nar-question">{{ CHAPTERS[5].question }}</p></div>
          </header>
          ...
          <div v-for="(reach, ruleIndex) in ruleReach" :id="`nar-${reach.rule.id}`" :key="reach.rule.id" class="nar-anchor space-y-4" :class="flashId === reach.rule.id && 'nar-flash'">
            <p class="nar-subnum">6.{{ ruleIndex + 1 }} · Business rule</p>
            <h3 class="text-xl tracking-tight text-highlighted">{{ reach.rule.title }}</h3>
            <div class="nar-pullquote"><BlrProse :text="reach.rule.statement" size="base" /></div>
            <div v-if="reach.rule.rationale" class="space-y-1"><p class="nar-label">Rationale</p><BlrProse :text="reach.rule.rationale" /></div>
            <div v-if="reach.rule.intent" class="space-y-1"><p class="nar-label">Intent</p><BlrProse :text="reach.rule.intent" /></div>
            <div class="space-y-2">
              <p class="nar-label">Directly attached — authored on the rule</p>
              <div class="flex flex-wrap gap-1.5">
                <button v-for="entity in reach.direct" :key="entity.id" type="button" class="nar-chip" @click="jumpToEntity(entity)">
                  <BlrKind :kind="entity.kind" :labelled="false" size="xs" />{{ entity.title }}
                </button>
                <span v-if="!reach.direct.length" class="text-xs italic text-dimmed">Nothing directly attached.</span>
              </div>
            </div>
            <div class="space-y-2">
              <p class="nar-label">Derived reach — computed from the model, not authored</p>
              <div class="flex flex-wrap gap-1.5">
                <button v-for="entity in reach.derived" :key="entity.id" type="button" class="nar-chip nar-chip--derived" @click="jumpToEntity(entity)">
                  <BlrKind :kind="entity.kind" :labelled="false" size="xs" />{{ entity.title }}
                </button>
                <span v-if="!reach.derived.length" class="text-xs italic text-dimmed">No reach beyond its direct attachments.</span>
              </div>
            </div>
            ...
            <BlrProse v-if="reach.rule.supportingContent" :text="reach.rule.supportingContent" class="text-dimmed" />
            <BlrRefs :references="reach.rule.references" />
          </div>
          <p v-if="!workspace.rules.length" class="text-sm italic text-dimmed">The model declares no Business Rules.</p>
        </section>

        <!-- ================================================== 7 · Colophon -->
        <section id="nar-chapter-colophon" data-chapter="colophon" class="nar-chapter space-y-8">
          <header class="nar-head">
            <span class="nar-num">7</span><div class="min-w-0"><h2 class="nar-title">{{ CHAPTERS[6].title }}</h2><p class="nar-question">{{ CHAPTERS[6].question }}</p></div>
          </header>
          ...
          <figure v-if="workspace.references.length" class="space-y-2">
            <figcaption class="nar-caption">Every reference in the model, with the entity that cites it.</figcaption>
            <div class="overflow-x-auto">
              <table class="nar-table">
                ...
                <tbody>
                  <tr v-for="(group, index) in workspace.references" :key="index">
                    ...
                    <td class="max-w-72">
                      ...
                      <span v-else class="block truncate font-mono text-xs text-toned" :title="group.reference.target">{{ group.reference.title || group.reference.target }}</span>
                    </td>
          ...
          <figure v-if="workspace.pairs.length" class="space-y-2">
            <figcaption class="nar-caption">Every declared access scope, with the entities that declare it directly (derived counts; Scenarios with no scope of their own inherit their Journey's and are not counted here).</figcaption>
            <div class="overflow-x-auto">
              <table class="nar-table">
                <thead>
                  <tr>
                ...
                <tbody>
                  <tr v-for="usage in pairUsage" :key="usage.pair.key">
                    <td class="text-toned">{{ usage.pair.interfaceTitle }}</td><td class="text-dimmed">{{ usage.pair.experienceTitle || '— direct' }}</td>
          ...
          <p class="border-t pt-4 font-mono text-xs leading-relaxed text-dimmed blr-hairline">Generated by {{ workspace.identity.generator.name }} v{{ workspace.identity.generator.version }} · {{ workspace.identity.generatedAt }} · schema {{ workspace.identity.schemaVersion }} · {{ workspace.identity.referenceProfile }} reference profile<template v-if="workspace.identity.license"> · {{ workspace.identity.license }}</template></p>
        </section>
      </article>

      <!-- Chapter rail: slim, sticky, scrollspy-highlighted on xl. -->
      <nav class="blr-no-print hidden xl:block" aria-label="Chapters">
        <ol class="sticky top-[calc(var(--blr-chrome)+1.5rem)] space-y-0.5 border-s border-default ps-1 pt-8">
          <li v-for="(chapter, index) in CHAPTERS" :key="chapter.id">
            <button type="button" class="flex w-full items-baseline gap-2 rounded-md px-2 py-1.5 text-start text-xs transition" :class="activeChapter === chapter.id ? 'bg-elevated font-medium text-highlighted' : 'text-dimmed hover:text-toned'" :aria-current="activeChapter === chapter.id ? 'true' : undefined" @click="scrollToChapter(chapter.id)">
              <span class="font-mono text-[10px] tabular-nums">{{ index + 1 }}</span>{{ chapter.title }}
            </button>
          </li>
        </ol>
...
/* Chapter heads: a large light numeral beside the title and its question. */
.nar-head { display: flex; align-items: baseline; gap: 1.25rem; border-top: 2px solid var(--ui-border-accented); padding-top: 1.5rem; }
.nar-num { font-size: 4rem; font-weight: 200; line-height: 0.9; color: color-mix(in srgb, var(--ui-text-dimmed) 55%, transparent); font-variant-numeric: tabular-nums; }
.nar-title { font-size: 1.75rem; font-weight: 500; letter-spacing: -0.02em; color: var(--ui-text-highlighted); }
.nar-question { margin-top: 0.25rem; font-style: italic; color: var(--ui-text-dimmed); }
.nar-sub { font-size: 1.125rem; font-weight: 500; letter-spacing: -0.01em; color: var(--ui-text-highlighted); }

.nar-label,
.nar-subnum { font-family: var(--font-mono); font-size: 0.625rem; letter-spacing: 0.12em; text-transform: uppercase; color: var(--ui-text-dimmed); }
.nar-caption { font-size: 0.75rem; line-height: 1.6; color: var(--ui-text-dimmed); }
...
.nar-pullquote :deep(p) { font-size: 1.25rem; font-weight: 350; line-height: 1.6; color: var(--ui-text-highlighted); }

/* Entity mention chips; derived reach is dashed so authored stays distinct. */
.nar-chip { display: inline-flex; align-items: center; gap: 0.375rem; border: 1px solid var(--ui-border); border-radius: 999px; padding: 0.125rem 0.625rem; font-size: 0.75rem; color: var(--ui-text-toned); transition: background 0.15s ease, border-color 0.15s ease; }
.nar-chip:hover { background: var(--ui-bg-elevated); }
.nar-chip--open { border-color: var(--ui-primary); }
.nar-chip--derived { border-style: dashed; color: var(--ui-text-dimmed); }
...
/* Printed tables: hairline rows, mono headings, generous cell air. */
.nar-table { width: 100%; border-collapse: collapse; font-size: 0.8125rem; }
.nar-table thead th { border-bottom: 1px solid var(--ui-border-accented); padding: 0.375rem 0.625rem; font-family: var(--font-mono); font-size: 0.625rem; font-weight: 500; letter-spacing: 0.1em; text-align: start; text-transform: uppercase; color: var(--ui-text-dimmed); vertical-align: bottom; }
.nar-table tbody td,
...
.nar-matrix-note { border-inline-start: 2px solid var(--ui-primary); padding-inline-start: 0.75rem; font-size: 0.8125rem; line-height: 1.6; color: var(--ui-text-toned); }
