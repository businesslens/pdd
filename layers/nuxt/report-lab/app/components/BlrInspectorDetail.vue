...
const facts = computed<Fact[]>(() => {
  switch (props.entity.kind) {
    ...
    case 'capability': return [
      ...
      { label: 'Journeys', value: asCapability.value.journeyIds.length }
    ]
    case 'journey': return [
    ...
    case 'scenario': return [
      { label: 'Journey', value: asScenario.value.journeyTitle },
...
const accessNote = computed(() => {
  if (props.entity.kind === 'scenario' && !availability.value.length) {
    return 'Applies to every Interface and Experience scope declared by its Journey.'
  }
  ...
  switch (props.entity.kind) {
    ...
    case 'screen':
      groups.push(group('Authored', [
        ...
        row('Scenarios', 'scenario', asScreen.value.scenarioIds)
      ]))
      groups.push(group('Derived', [row('Reached from Journeys', 'journey', asScreen.value.journeyIds)]))
      break
    case 'domain':
      groups.push(group('Derived', [
    ...
    case 'journey':
      groups.push(group('Authored', [
        row('Actors', 'actor', asJourney.value.actorIds),
        row('Capabilities', 'capability', asJourney.value.capabilityIds)
      ]))
      groups.push(group('Derived', [
      ...
      break
    case 'scenario':
      groups.push(group('Authored', [row('Journey', 'journey', [asScenario.value.journeyId])]))
      groups.push(group('Derived', [
    ...
    case 'rule':
      groups.push(group('Authored', [
        ...
        row('Scenarios', 'scenario', asRule.value.scenarioIds)
      ]))
      {
      ...
      <div v-if="availability.length" class="space-y-2">
        ...
        <div class="divide-y divide-default overflow-hidden rounded-xl border border-default">
          <div v-for="pair in availability" :key="pair.key" class="flex flex-wrap items-center gap-2 px-3.5 py-2.5 text-sm">
            <button type="button" class="font-medium text-highlighted hover:text-primary" @click="selectId(pair.interfaceId)">{{ pair.interfaceTitle }}</button>
            <template v-if="pair.experienceId">
              <UIcon name="i-lucide-chevron-right" class="size-3.5 text-dimmed" />
              <button type="button" class="text-default hover:text-primary" @click="selectId(pair.experienceId)">{{ pair.experienceTitle }}</button>
            </template>
      ...
      <div v-if="entryPoints.length" class="space-y-2">
        ...
        <div class="divide-y divide-default overflow-hidden rounded-xl border border-default">
          <div v-for="point in entryPoints" :key="`${point.interfaceId}-${point.path}`" class="grid gap-1 px-3.5 py-2.5 sm:grid-cols-[11rem_1fr] sm:items-center">
            <button type="button" class="truncate text-start text-sm font-medium text-highlighted hover:text-primary" @click="selectId(point.interfaceId)">{{ point.interfaceTitle }}</button>
        ...
        <div v-for="relation in relationGroup.rows" :key="relation.label" class="space-y-2">
          ...
          <div class="flex flex-wrap gap-1.5">
            ...
            >
              ...
              <span class="truncate">{{ workspace.byId.get(id)?.title ?? id }}</span>
            </button>
          </div>
        </div>
      </div>
