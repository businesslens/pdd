...
/**
 * The contextual topology the brief requires — one entity's neighbourhood,
...
const props = withDefaults(defineProps<{
  ...
  focusId: string
  /** Hide the explanation strip when the host design provides its own. */
  explain?: boolean
  direction?: 'LR' | 'TB'
}>(), {
  explain: true,
...
const emit = defineEmits<{
  ...
  /** The reader asked for the full entity content in the host design. */
...
const focusEntity = computed(() => props.workspace.byId.get(focus.value) ?? null)
const selectedEntity = computed(() => (selectedId.value && props.workspace.byId.get(selectedId.value)) || null)
...
const presentKinds = computed<ReportEntityKind[]>(() => {
  ...
  for (const rootId of [focus.value, ...expandedIds.value]) {
    for (const id of neighbourIds(props.workspace, rootId)) {
      kinds.add(props.workspace.byId.get(id)!.kind)
    }
  }
...
const selectedExpandable = computed(() => (
  ...
  && selectedEntity.value.id !== focus.value
  && !expandedIds.value.includes(selectedEntity.value.id)
  && neighbourIds(props.workspace, selectedEntity.value.id).length > 0
))
...
function refocus(entityId: string) {
  ...
  emit('select', props.workspace.byId.get(entityId) ?? null)
}
...
function expandSelected() {
  ...
  expandedIds.value = [...expandedIds.value, selectedEntity.value.id]
}
  ...
  <div class="flex h-full min-h-0 flex-col">
    <div class="flex flex-wrap items-center gap-x-3 gap-y-1.5 border-b border-default px-3 py-2">
      <UButton
        ...
        variant="ghost"
        size="xs"
        :label="`Back to ${workspace.byId.get(history.at(-1)!)?.title ?? 'previous'}`"
        @click="goBack"
      />
