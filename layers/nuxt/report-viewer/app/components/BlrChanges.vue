...
<template>
  <div class="space-y-5" data-changes data-history>
    ...
    <BlrReviewRepository v-if="changes.repository" v-model:path="path" :diff="changes.repository" :base="baseId" :target="targetId" :before="before" :after="after" :model-notice="changes.modelNotice" :load-file="loadRepositoryFile" :resource-reading-open="resourceReadingOpen" @inspect="(key, state) => emit('inspect', key, state)" />
  </div>
</template>
