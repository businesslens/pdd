...
<template>
  <div class="min-w-0 space-y-5">
    ...
    <template v-if="tab === 'overview'">
      ...
      <div class="blr-meta flex items-center gap-1">
        <span>Report format {{ workspace.identity.schemaVersion }}</span>
        <BlrTerm slug="report-format" :text="`Report format ${workspace.identity.schemaVersion}`" icon-only />
      </div>
    </template>
