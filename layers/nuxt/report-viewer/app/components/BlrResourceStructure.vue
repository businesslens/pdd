...
import { structureChildren, treeBranchKeys } from '../utils/collectionChildren'
...
<template>
  <section class="space-y-3" aria-label="Structure" data-resource-structure>
    ...
    <div class="overflow-hidden rounded-xl border border-default bg-elevated/20 px-3 py-2">
      <BlrResourceTree v-model:expanded="expanded" :nodes="nodes" :label="`${resource.title} structure`" @open="emit('open', $event)" />
    </div>
  </section>
</template>
