...
/**
 * Where material lives decides what following it does: a repository file opens
 * in this report and leaves with the repository when a Blueprint is exported;
 * an external link opens elsewhere and travels. Each origin is its own section,
 * grouped by kind within it.
 */
type Origin = 'internal' | 'external'
const ORIGINS: Array<{ origin: Origin, label: string, icon: string }> = [
  { origin: 'internal', label: 'In this repository', icon: 'i-lucide-folder' },
  { origin: 'external', label: 'External', icon: 'i-lucide-globe' }
]
const originOf = (reference: ReportReference): Origin => isExternal(reference.target) ? 'external' : 'internal'

function kindGroups(origin: Origin): Node[] {
  return Object.entries(KIND_LABEL).flatMap(([kind, label]) => {
    ...
    // Kind groups are keyed by origin, so each section keeps its own expansion.
    return children.length ? [{ value: `${origin}:${kind}`, kind: kind as ReportReference['kind'], label, count: children.length, children }] : []
  })
}
const sections = computed(() => ORIGINS
  .map(section => ({ ...section, items: kindGroups(section.origin) }))
  .map(section => ({ ...section, count: section.items.reduce((total, group) => total + group.count!, 0) }))
  .filter(section => section.items.length))
const groups = computed(() => sections.value.flatMap(section => section.items))
const branchKeys = computed(() => groups.value.flatMap(group => [group.value, ...group.children!.filter(item => item.children).map(item => item.value)]))
const expanded = useBlrReferenceExpansion(computed(() => props.scope), branchKeys, computed(() => props.defaultExpanded ? groups.value.map(item => item.value) : []))
...
<template>
  <div v-if="references.length" class="min-w-0 space-y-3">
    ...
    >
      <p class="flex items-center gap-1.5 text-sm font-medium text-highlighted">
        ...
        <span class="blr-meta">{{ section.count }}</span>
      </p>
