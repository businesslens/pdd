  ...
  <section class="min-w-0 space-y-3" aria-label="Source areas" data-coverage-sources>
    ...
    <div class="grid grid-cols-3 gap-2 @xl/coverage:gap-3" role="group" aria-label="Source summary filters">
      <button
        ...
        class="flex min-w-0 cursor-pointer flex-col items-start gap-0.5 rounded-lg border p-2 text-start transition-colors focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-primary @xl/coverage:px-3"
        :class="filter === kind ? 'border-primary bg-primary/5' : 'border-default hover:border-accented hover:bg-elevated/40'"
        @click="filter = filter === kind ? 'all' : kind"
      >
        <span class="flex w-full items-center gap-2">
          <span class="text-xl font-semibold tabular-nums text-highlighted" data-coverage-summary-count>{{ workspace.coverage[kind].length }}</span>
          <span class="hidden text-sm font-medium text-highlighted @xl/coverage:inline">{{ label }}</span>
          <UIcon name="i-lucide-check" class="ms-auto size-4 shrink-0 text-primary" :class="filter !== kind && 'invisible'" />
        </span>
        <span class="block text-xs font-medium text-highlighted @xl/coverage:hidden">{{ label }}</span>
    ...
    <BlrRepositoryTree v-model:expanded="expanded" :path="path" :nodes="tree" root-label="Repository" :empty-message="filter === 'all' ? 'No repository paths recorded.' : 'No recorded paths match this filter.'" @update:path="value => { if (value) emit('selectPath', value) }">
      <template #indicators="{ node }">
        <template v-for="(count, kind) in counts.get(node.value)" :key="kind">
          <span v-if="count" class="rounded border border-default px-1 text-[10px]" :data-coverage-kind="kind" :title="`${count} ${annotationLabels[kind].toLowerCase()} ${node.directory ? 'at or below this folder' : 'at this file'}`">{{ annotationLabels[kind] }}<span v-if="node.directory || count > 1" class="ms-1">{{ count }}</span></span>
        </template>
      </template>
    </BlrRepositoryTree>
    <section v-if="unlocated.length" class="space-y-3 border-t border-default pt-4" aria-label="No location recorded">
      ...
      <ul class="space-y-4">
        <li v-for="area in unlocated" :key="area.description" class="space-y-2" data-coverage-entry>
          <span class="rounded border border-default px-1 text-[10px]">{{ labels[area.kind] }}</span>
