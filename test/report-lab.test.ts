...
describe('report-lab brief-driven designs', () => {
  ...
  it('auditions one shared card system and keeps completeness in the inspector', () => {
    ...
    expect(workbench).toContain('#detail-after="{ entity }"')
    ...
    expect(inspector).toContain('<slot name="detail-after"')
    const scenarioDrilldown = source('app/components/BlrWorkbenchScenarioDrilldown.vue')
    expect(scenarioDrilldown).toContain("entity.kind === 'capability'")
    expect(scenarioDrilldown).toContain("entity.kind === 'journey'")
    expect(scenarioDrilldown).toContain('v-for="scenario in scenarios"')
    expect(scenarioDrilldown).toContain('<UCollapsible')
    expect(scenarioDrilldown).toContain('<BlrInspectorDetail')
