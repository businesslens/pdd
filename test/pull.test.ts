...
function reportResponse(canonicalName = 'fixture-shop'): Response {
  return new Response(JSON.stringify(report), {
    status: 200,
    headers: {
      'content-type': 'application/vnd.businesslens.report+json; version=16',
...
describe('pull', () => {
  it('pulls anonymously from the default catalog and expands the model', async () => {
    ...
    expect((requested?.init.headers as Record<string, string>).accept).toContain('version=16')
  ...
  it('asks the catalog for the schema\'s own report version and refuses another', async () => {
    ...
    expect(await runPull(target, 'fixture-shop', { force: false }, { fetch, env: {} })).toBe(1)
    expect(errors[0]).toContain('The catalog serves Product Report version 11; this CLI reads version 16 only')
    expect(existsSync(join(target, '.businesslens'))).toBe(false)
  })
  ...
  it('names the schema version of a report it cannot read, in one sentence', async () => {
    ...
    expect(await runPull(target, 'fixture-shop', { force: false }, { fetch, env: {} })).toBe(1)
    expect(errors[0]).toBe('This is a Product Report of schema version 12.0.0; only 16.0.0 is accepted, and there is no compatibility reader. Export it again with a current businesslens.')
  })
