...
function compose(variant, { size, fit, plate }) {
  const { box } = variant
  ...
  const elements = variant.elements.map((element, index) => {
    ...
    if (!light || !dark || light === dark.trim()) return element
    swaps.push(`.d${index}{fill:${dark}}`)
    return element.replace(/^<(\w+)/, `<$1 class="d${index}"`)
  })
...
for (const mark of MARK_VARIANTS) {
  ...
  const tab = compose(variant, { size: 64, fit: variant.container ? 1 : 0.88, plate: null })
  ...
  console.log(`${mark.id.padEnd(4)} ${mark.name.padEnd(9)} from ${mark.file}`)
}
