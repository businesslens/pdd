...
try {
  for (const consumer of consumers) {
    ...
    try {
      ...
      await page.goto(`${origin}/?s=domain&t=map`)
      ...
      const ssr = await fetch(`${origin}/?s=entity&t=relationships&multi=1`).then(response => response.text())
      expect(ssr).toContain('Arranging diagram')
      expect(ssr).toContain('knows M:N')
      await page.goto(`${origin}/?s=entity&t=relationships&multi=1`)
      ...
      /* The catalog host names the section `tab`. A link shared before the
         collections existed opens the Overview rather than guessing a home. */
      ...
      await page.goto(`${origin}/?catalog=1&tab=interface&t=map`)
      await expect(page.locator('[data-flow-ready="true"]')).toHaveCount(1)
      await page.locator('.blr-navitem').filter({ hasText: /^Entities/ }).click()
      await page.locator('.blr-surface-tab').filter({ hasText: /^Relationships/ }).first().click()
      ...
      await expect(page.locator('.blr-surface-tab[data-current="true"]')).toContainText('Map')
