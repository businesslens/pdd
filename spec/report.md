...
- `build/report.json` — portable Product Report v12 generated by
  `blueprint export`.
...
`build/report.json` is a Product Report with `schemaVersion: "12.0.0"`. It
...
The raw `supportingContent` string field is not part of Product Report v12.
...
Product Report v11 added an `entities` collection and its count. Each Entity
...
Product Report v12 replaces the step's nullable `entityId` and `entityState`
with that array, and adds a `readEntityIds` array beside it. Each `changes`
entry is `{ entityId, effect, state }`, where `effect` is `creates`, `changes`,
or `removes` — resolved on the wire rather than defaulted, so a reader never has
to know which value the folder omits — and `state` is nullable. One Step may
carry several entries, and never two for one Entity. `readEntityIds` lists the
Entities the step picks, inspects, or displays without changing; it stays a bare
id list, no Entity appears in both arrays of one step, and a read never counts
as a change for the rule that an Entity nothing changes and no Screen presents
is invalid.

Validation resolves every edge, exactly as it resolves Actor and Interface
relations: a `relations` target, a transition's `capabilityId`, a Capability or
Screen `entityIds` member, and every step change's `entityId` must all name an
Entity in the same report; a relation never reads `many-to-one`, and two
Entities never declare relations at each other; a transition's `from` and `to`
must be that Entity's own state names and its `capabilityId` must be a
Capability listing that Entity; a step change's `state` must be a state of the
named Entity, reached by some transition of the step's Capability when the
effect is `changes`, unconstrained by transitions when it is `creates`, and
absent when it is `removes`; and an Entity no Capability changes and no Screen
...
Product Report v12 stores `capabilityScenarios` and `journeyScenarios` as
...
Product Report v12 stores the Context-only schema the folder format uses.
...
ordered `contexts` array. `actorId` is non-null exactly for `actor` Steps.
...
other local References. Asset binaries are not part of Product Report v12.
...
Product Report v12 is the only accepted report version — there is no
