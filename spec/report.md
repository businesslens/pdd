...
- `build/report.json` — portable Product Report v14 generated by
  `blueprint export`.
...
`build/report.json` is a Product Report with `schemaVersion: "14.0.0"`. It
...
The raw `supportingContent` string field is not part of Product Report v14.
...
Product Report v14 has one collection for things: `entities`. There is no
...
Product Report v14 stores `capabilityScenarios` and `journeyScenarios` as
...
Product Report v14 stores the Context-only schema the folder format uses.
...
other local References. Asset binaries are not part of Product Report v14.
...
Entity, and Capability consistency holds. Product Report v14 is the only
...
alone** — `version=14` for `schemaVersion: "14.0.0"`. The major is the whole
